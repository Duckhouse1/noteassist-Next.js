...
const extractInfoBasedOnAction = async (noteContent: string,Transcript:string ,action: Action): Promise<OpenAIResponse> => {
    const response = await fetch('/api/AzureOpenAI', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ noteContent,Transcript ,action })
    });
