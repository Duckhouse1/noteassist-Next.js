...
async function exchangeCodeForToken(params: {
    ...
    body.set("scope", SHAREPOINT_SCOPES);
...
export async function GET(req: NextRequest) {
    ...
    const cookie = req.cookies.get("sharepoint_oauth")?.value;
    if (!cookie) {
    ...
    const redirectUri = `${process.env.NEXTAUTH_URL}/api/integrations/SharePoint/callback`;
    ...
    try {
        ...
        res.cookies.set("sharepoint_oauth", "", { path: "/", maxAge: 0 });
        return res;
    } catch (err: unknown) {
