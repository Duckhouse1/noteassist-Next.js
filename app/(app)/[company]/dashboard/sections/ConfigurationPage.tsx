...
import { AzureDevopsSettings, DEFAULT_AZURE_DEVOPS_SETTINGS, AzureDevopsConfig } from "./configElements.tsx/AzureDevopsConfig";
import { GeneralConfig } from "./configElements.tsx/GeneralConfig";
import { OutlookSettings, DEFAULT_OUTLOOK_SETTINGS, OutlookConfig } from "./configElements.tsx/OutlookConfig";
import { SharePointSettings, DEFAULT_SHAREPOINT_SETTINGS, SharePointConfig } from "./configElements.tsx/SharePointConfig";
...
export function ConfigurationPage({ value, onChange, onSave, company, connections }: {
  ...
  function setActionEnabled(key: ActionKey, enabled: boolean) {
    setSaved(false);
    onChange({ ...value, enabledActions: { ...value.enabledActions, [key]: enabled } });
  }

  function toggleProvider(provider: IntegrationOptions) {
    setSaved(false);
    onChange({ ...value, enabledProviders: toggleInArray(value.enabledProviders, provider.title) });
  }
  ...
  // Compare by id — never by object reference
      ...
      <div className="bg-white border-b border-slate-200 px-8 py-5">
        <div className="mx-auto max-w-7xl 2xl:max-w-[1600px] flex items-center justify-between">
          <div>
            ...
            <h1 className="text-xl font-bold tracking-tight text-slate-900">Configuration</h1>
          </div>
          ...
          <div className="space-y-5">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <button
                ...
                onClick={() => setCurrentConfig("General")}
              >
                General
              </button>

              {connections.map((connect) => (
                <button key={connect.id}
                  className={[
                    "rounded-lg px-4 py-1.5 text-xs font-semibold transition-all focus:outline-none focus:ring-2 focus:ring-[#1E3A5F] focus:ring-offset-1",
                    activeId === connect.id
                      ? "bg-[#1E3A5F] text-white shadow-sm"
                  ...
                  onClick={() => setCurrentConfig(connect)}
                >
                  {connect.displayName}
                </button>
              ))}
              <div className="ml-auto">
                <div className="flex">
                  ...
                  <button
                    type="button"
                    onClick={handleSave}
                    className="cursor-pointer ml-6 rounded-lg bg-[#1E3A5F] px-3.5 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-[#16304F] focus:outline-none focus:ring-2 focus:ring-[#1E3A5F] focus:ring-offset-2"
                  >
                    Save changes
                  </button>
                </div>
            ...
            {currentConfig !== "General" && currentConfig.provider === "Azure-Devops" && (
              <AzureDevopsConfig
                connection={currentConfig}
                settings={getAzureSettings(value, currentConfig.id)}
                onChange={(next) => {
                  setSaved(false);
                  onChange({ ...value, azureDevops: { ...value.azureDevops, [currentConfig.id]: next } });
                }}
              />
            )}

            {currentConfig !== "General" && currentConfig.provider === "Outlook" && (
              <OutlookConfig
                connection={currentConfig}
                settings={getOutlookSettings(value, currentConfig.id)}
                onChange={(next) => {
                  setSaved(false);
                  onChange({ ...value, outlook: { ...value.outlook, [currentConfig.id]: next } });
                }}
              />
            )}

            {currentConfig !== "General" && currentConfig.provider === "SharePoint" && (
              <SharePointConfig
                connection={currentConfig}
                settings={getSharePointSettings(value, currentConfig.id)}
                onChange={(next) => {
                  setSaved(false);
                  onChange({ ...value, sharePoint: { ...value.sharePoint, [currentConfig.id]: next } });
                }}
              />
            )}
          </div>
          ...
          <aside className="space-y-4">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-sm font-bold text-slate-900 tabular-nums">{enabledCount}</span>
              </div>
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Active providers</p>
                <div className="flex flex-wrap gap-1.5">
                  {(value.enabledActions.integrations ? value.enabledProviders : []).map((p) => (
                    <span
                      key={p}
                  ...
                  {value.enabledActions.integrations && value.enabledProviders.length === 0 && (
                    <span className="text-xs text-slate-400">No providers selected.</span>
                  )}
                  {!value.enabledActions.integrations && (
                    <span className="text-xs text-slate-400">Integrations disabled.</span>
                  )}
                </div>
              </div>
            </div>
            ...
            {/* Connections card */}
            <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">Connections</p>
              <p className="text-xs text-slate-500 mb-4">Connect your workspace to external tools.</p>
                  ...
                  return (
                    <div
                      ...
                      className={[
                        ...
                        isConnected ? "border-emerald-200 bg-emerald-50/50" : "border-slate-200 bg-white",
                      ].join(" ")}
                    >
                      <div className="flex items-center gap-2.5">
                        <span className={["h-2 w-2 rounded-full", isConnected ? "bg-emerald-500" : "bg-slate-300"].join(" ")} />
                        <div>
                          <span className="text-xs font-semibold text-slate-800">{p.title}</span>
                          <p className="text-[10px] text-slate-500 mt-0.5">{isConnected ? "Ready to use" : "Not connected"}</p>
                        </div>
                      </div>
                      {isConnected ? (
                        <span className="text-[10px] font-bold text-emerald-600 uppercase tracking-wide">Connected</span>
                      ) : (
                        <Link
                          href={connectHref(p)}
